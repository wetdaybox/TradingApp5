...
def gen_sig(df,is_btc,params):
    ...
    for i in range(EMA_T,len(df)-1):
        ...
        if is_btc:
            ...
            m,b,_,dip=params; mv=df["price"].rolling(m).mean().iat[i]
...
def extract_Xy(pr,is_b):
    df = pd.DataFrame({"price":pr})
    return gen_sig(compute_ind(df), is_b, btc_params if is_b else xrp_params)

# append real
for prices,is_b,p in [(list(btc_hist["price"].values[-90:])+[btc_p], True, btc_p),
                      (list(xrp_hist["price"].values[-90:])+[xrp_p], False, xrp_p)]:
...
for key,label,hist,(pr,ch),prob in [
    ...
    # state logic here (same as before)...
    ...
    # UI metrics, action buttons, deploy logic as before...
