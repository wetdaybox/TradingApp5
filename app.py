...
def load_live():
    js  = fetch_json("https://api.coingecko.com/api/v3/simple/price",
                     {"ids":"bitcoin,ripple","vs_currencies":"usd,btc","include_24hr_change":"true"}) or {}
    btc = js.get("bitcoin", {})
    xrp = js.get("ripple", {})
    return {
...
def train_clf(X, y):
    ...
    # fallback: no training or single-class
...
# ── Safe predict_proba ──
def safe_prob(clf, feat, label):
    if feat is None or len(feat)==0:
        return 0.0
    probs = clf.predict_proba(feat)[0]
    return probs[1] if probs.shape[0]>1 else 0.0

p_btc = safe_prob(clf_btc, today_feat(btc_hist), 1)
p_xrp = safe_prob(clf_xrp, today_feat(xrp_hist), 1)
...
sig_xrp  = (xrp_hist["price"].iat[-1] < xrp_hist["price"].rolling(int(xrp_params[0])).mean().iat[-1]) \
...
with st.expander("ℹ️ About"):
    ...
    2. **Min Order** must meet exchange minimum.  
    3. **Final Defaults** shows rule-based or ML override (p(win)≥80%).  
    4. **Grid Depth (Drop %)** = how far below price your grid bottom sits.  
    5. **Per-Order (BTC)** = BTC invested per grid level (green if ≥ min).  
    6. **Take-Profit** = target exit price per grid.  
