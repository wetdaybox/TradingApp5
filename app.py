...
def save_flags(f): pickle.dump(f, open(FLAGS_FILE, "wb"))
def load_ml_buffer():
    ensure_state_dir()
    if os.path.isfile(ML_BUF_FILE):
        return pickle.load(open(ML_BUF_FILE, "rb"))
    return {"X":[],"y":[],"ts":[]}
def save_ml_buffer(b): pickle.dump(b, open(ML_BUF_FILE, "wb"))
...
def generate_scenario(vol,reg,days=90):
    mapp = {"normal":(0,vol,None),"high-vol":(0,vol*2,None),
...
# ─── FIXED extract_Xy: build both price & return before compute_ind ───
def extract_Xy(prices,is_b):
    df = pd.DataFrame({"price": prices})
    df["return"] = df["price"].pct_change()*100
    df = df.dropna()  # ensure no NaN in return
    df = compute_ind(df)
...
# (The rest of the auto_state() & rendering code is identical to before...)
