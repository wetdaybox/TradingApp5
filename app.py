...
# Set up super detailed logging
logging.basicConfig(
    filename="trading_bot.log",
    level=logging.DEBUG,  # DEBUG level captures detailed info
...
class TradingBot:
    def __init__(self, ticker='AAPL', portfolio_value=100000, leverage=5, sma_window=50,
        ...
        self.portfolio_value = portfolio_value  # Tracks available cash
    ...
    def calculate_features(self, df):
        ...
        try:
            ...
            logging.debug(f"Feature calculation complete. Data shape: {df.shape}")
            return df.dropna()
        except Exception as e:
    ...
    def generate_signals(self, df):
        ...
        try:
            df = df.copy()
            full_index = df.index
            # Convert the weekday condition to a Series with the same index
            ...
            logging.debug(f"Signal generation complete. Signal sample:\n{df['Signal'].head()}")
            return df
        except Exception as e:
...
def simulate_leveraged_cumulative_return(df, leverage=5):
    ...
    try:
        ...
        # Log shapes and indices before resetting
