...
def backtest_btc(df, rsi_th, tp_mult, sl_pct):
    ...
    for i in range(EMA_TREND, len(df)-1):
        p = df["price"].iat[i]
        cond = (p > df["ema50"].iat[i]) and (df["sma5"].iat[i] > df["sma20"].iat[i]) and (df["rsi"].iat[i] < rsi_th)
        if not cond: 
            continue
        ret, vol = df["return"].iat[i], df["vol14"].iat[i]
        if ret < vol: 
            continue
        drop = vol if ret <= 2*vol else 2*vol
        if df["price"].iat[i+1] > p: 
            wins += 1
        else: 
            losses += 1
...
def backtest_xrp(df, mean_d, bounce_pct, sl_pct, min_dip):
    ...
    for i in range(mean_d, len(df)-1):
        ...
        if not (p < m and gap >= min_dip and vol_up):
            continue
        tp = gap/100 * p * (bounce_pct/100)
        if df["price"].iat[i+1] >= p + tp:
            wins += 1
        else:
            losses += 1
    return wins/(wins+losses) if (wins+losses) else 0.0
...
btc_df_s = pd.DataFrame(btc_rows, columns=["rsi","tp","sl","win_rate"])
...
for m in (5,10,15):
    for b in (50,75,100):
        for sl in (25,50,75):
            for md in (1.0,1.5):
                xrp_rows.append((m, b, sl, md, backtest_xrp(xrp_hist, m, b, sl, md)))
xrp_df_s = pd.DataFrame(xrp_rows, columns=["mean","bounce","sl","min_dip","win_rate"])
...
st.sidebar.write(f"BTC{' (ML)' if ml_btc else ''}: RSI<{int(btc_def[0])}, TP×{btc_def[1]}, SL{btc_def[2]}% (pred {btc_pred:.0%})")
...
drop  = vol14 if ret24 < vol14 else (2*vol14 if ret24 > 2*vol14 else ret24)
if drop:
...
with st.expander("ℹ️ About"):
    st.markdown("""
    • Backtests auto-tune BTC & XRP grid params for ≥70 % win-rate over 90 d.  
    • ML models trained on those backtests override defaults when predicted ≥70 %.  
    • “✅ ML Override Active” flags the new settings.  
    • Copy these grid bounds into your Crypto.com grid bot manually.
    """)
