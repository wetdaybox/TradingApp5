...
# <<< **CRITICAL**: define override *before* auto_state uses it
...
def generate_scenario(vol,reg,days=90):
    mapping={"normal":(0,vol,None),"high-vol":(0,vol*2,None),
             "crash":(-0.002,vol*3,(-0.3,)),"rally":(0.002,vol*1.5,(0.3,)),
...
for prices,is_b in [
    ...
    df["return"]=df["price"].pct_change()*100; df=df.dropna()
...
for key,label,hist,(pr,ch),prob in [
    ...
    low,up,tp,sl,grids,rec,act=auto_state(key,hist,pr,ch,prob,st.session_state.get(f"cont_low_{key}",pr),st.session_state.get(f"cont_up_{key}",pr),st.session_state.get(f"cont_grids_{key}",30))
    ...
    if act=="Not Deployed":
        st.warning("⚠️ Waiting to deploy—adjust settings or override."); continue
    c1,c2=st.columns(2)
    if st.session_state.mode=="new":
        c1.metric("Grid Levels",grids); c2.metric("Recommended",rec)
    else:
        c1.metric("Grid Levels",grids); c2.write("")
