#!/usr/bin/env python3
"""
Autonomous Adaptive Trading System – Streamlit Version (Final Revised with Explicit Squeeze and Alignment)
...
Features:
  ...
  - Forces the daily returns and signal to be one-dimensional Series using .squeeze(),
...
def calculate_trade_recommendation(df, portfolio_value=10000, leverage=5, stop_loss_pct=0.05, take_profit_pct=0.10):
    ...
    Use dynamic position sizing and a leverage factor.
    """
    latest = df.iloc[-1]
...
def simulate_leveraged_cumulative_return(df, leverage=5):
    ...
    Steps:
      1. Ensure the DataFrame is reindexed to a complete business-day date range.
      2. Calculate daily returns.
      3. Reindex the 'signal' column to match the full date range, filling missing values with 0.
      4. Force both 'daily_return' and 'signal' to be one-dimensional float Series using .squeeze().
      5. Explicitly align them using .align() on axis=0.
      6. Multiply them elementwise.
      7. Calculate cumulative return.
      8. Output debugging information.
    """
    # Uniform data preparation (the DataFrame should already have a complete business-day index if prepared)
    full_range = df.index
    # Calculate daily returns
    df['daily_return'] = df['price'].pct_change().fillna(0).astype(float)
    
    # Ensure 'signal' exists and is float, then reindex to full_range
    ...
    # Convert both columns to one-dimensional Series using squeeze()
    dr = df['daily_return'].squeeze()
    sig = df['signal'].squeeze()
    
    # Explicitly align them on axis=0
    ...
    # Multiply elementwise using the * operator
...
class TradingBot:
    ...
    def run_cycle(self):
        ...
        # Prepare uniform data (and save it)
