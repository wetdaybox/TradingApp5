...
Improved live-fetch diagnostics and multiple fallbacks (CoinGecko, Binance, Coinbase, Kraken).
Full single-file replacement — paste & redeploy.
"""
...
# ---------------- Networking helpers (improved diagnostics) ----------------
def fetch_json(url, params=None, headers=None, timeout=12):
    """
    Robust GET with retries. Returns tuple (json_or_None, status_code_or_None, text_snippet_or_None, error_or_None)
    """
    headers = headers or {"User-Agent": "GridBot/1.0 (+https://example.com)"}
    backoff = 1.5
    last_err = None
    for i in range(MAX_RETRIES):
        try:
            ...
            try:
                ...
                return None, status, text, f"Non-JSON response from {url}"
...
def load_hist(coin_id, vs_currency):
    """
    Load H_DAYS history. Tries CoinGecko range -> market_chart -> Binance klines fallback.
    Returns DataFrame with 'price' and 'return' columns or empty DF.
    """
    now_s = int(time.time())
    from_s = now_s - H_DAYS * 86400

    # prefer coin gecko range endpoint first
    ...
    if not prices:
        ...
        if symbol:
            kb, ks, kt, ke = fetch_json("https://api.binance.com/api/v3/klines",
                                       {"symbol": symbol, "interval": "1d", "limit": H_DAYS})
...
def load_live():
    """
    Try a list of endpoints for live prices. Returns structure:
    {"BTC": (price, change_pct_or_None), "XRP": (price, None), "source": name, "errors": [messages]}
    """
    errors = []
    # 1) CoinGecko
    ...
    if err:
        errors.append(f"CoinGecko simple/price error: {err}")
    elif not j or "bitcoin" not in j or "usd" not in j.get("bitcoin", {}):
        errors.append(f"CoinGecko simple/price unexpected response status={status} text={(text or '')[:200]}")
    else:
        ...
        if e2:
            ...
            errors.append(f"CoinGecko ripple unexpected response status={s2} text={(t2 or '')[:200]}")
        else:
            try:
    ...
    try:
        ...
        if eb or ex:
            if eb: errors.append(f"Binance BTC error: {eb}")
            if ex: errors.append(f"Binance XRP error: {ex}")
        else:
            if jb and jx and "lastPrice" in jb and "lastPrice" in jx:
                ...
                errors.append(f"Binance unexpected response for BTC/XRP: BTC_keys={list(jb.keys()) if jb else 'None'}")

    except Exception as e:
        errors.append(f"Binance fetch exception: {e}")

    # 3) Coinbase Pro (public ticker) fallback
    try:
        ...
        if ecb or ecx:
            if ecb: errors.append(f"Coinbase BTC error: {ecb}")
            if ecx: errors.append(f"Coinbase XRP error: {ecx}")
        else:
            if jcb_b and jcb_x and "price" in jcb_b and "price" in jcb_x:
                try:
                    btc_price = float(jcb_b["price"])
                    # Coinbase doesn't give 24h change in same endpoint easily; set None
    ...
    # 4) Kraken public ticker as final fallback
    try:
        ...
        if ek_b or ek_x:
            if ek_b: errors.append(f"Kraken BTC error: {ek_b}")
            if ek_x: errors.append(f"Kraken XRP error: {ek_x}")
        else:
            # Kraken response parsing is a bit nested; try-except
            try:
    ...
    # All live attempts failed => return NaN prices with errors attached
    return {"BTC": (np.nan, None), "XRP": (np.nan, None), "source": "none", "errors": errors}

# ---------------- Indicators & ML helpers ----------------
def compute_ind(df):
...
# ---------------- Load history & compute indicators ----------------
diag_msgs = []
...
# ---------------- Bootstrap classifier on history if available ----------------
try:
...
# partial_fit if real fraction >=10%
...
# ---------------- Predictions & bot logic omitted for brevity (same as previous) ----------------
# NOTE: The remaining program logic (compute_grid_levels, auto_state, rendering, persistence)
# should be identical to your prior working logic. For brevity in this message I focus on the live fetch fixes.
# In your deployed app please keep the rest of the previously-provided complete code blocks for rendering and logic.
